...
class PrimerSetFormatter:
    ...
    def format_primer_set(arrangements, sequence_set):
        ...
        for j, seq in enumerate(sequence_set):
                ...
                pair = pset.set[v[j]]
                s += PrimerSetFormatter.format_primer_pair(pair, pset.name, amplicon)
    ...
    def format_primer_pair(primer_pair, name, amplicon):
        ...
        if primer_pair.predefined:
            s += '<a class="collapsed" data-toggle="collapse" data-parent="#tablist" href="#collapse' + name + '" aria-expanded="false" aria-controls="collapse' + name +'">\n'\
            + name + ": " + primer_pair.fwd.sequence.upper() + " - " + primer_pair.rev.sequence.upper() + '&emsp;Predefined'\
            + '\n</a>\n'\
            + '</h4>\n'\
            + '</div>\n'\
            + '</div>\n'
            ...
            s += '<a class="collapsed" data-toggle="collapse" data-parent="#tablist" href="#collapse' + name + '" aria-expanded="false" aria-controls="collapse' + name +'">\n'\
            + name + " (sequence " + str(amplicon + 1) + "): " + primer_pair.fwd.sequence.upper() + " - " + primer_pair.rev.sequence.upper() + '&emsp;Product Size: ' + str(primer_pair.product_size)\
            + '\n</a>\n'\
            + '</h4>\n'\
            + '</div>\n'\
            ...
            + '<td>Position: ' + str(primer_pair.fwd.location.start) + " - " + str(primer_pair.fwd.location.end) +'</td>'\
            ...
            + '<table style="width:100%"><tr>' \
            + '<td>Position: ' + str(primer_pair.rev.location.start) + " - " + str(primer_pair.rev.location.end) +'</td>'\
        ...
        for i, key in enumerate(sequence_set.keys()[::-1]):
            ...
            for seq in sequences:
                ...
                for i, primer_fwd in enumerate(seq.primer_set_fwd.set):
                    str_f += "<td>Fwd " + str(i) + ":</td><td>" + primer_fwd.sequence + "</td><td> Position: " \
                             + str(primer_fwd.location.start) + " - " + str(primer_fwd.location.end) + "</td>" \
                             + "<td>Length: " + str(len(primer_fwd)) + "</td><td>Tm: " + str(primer_fwd.tm) + "</td>" \
                             + "<td>GC%: " + str(primer_fwd.gc_content) + "</td>" \
                             + "<td>Any: " + str(primer_fwd.any) + "</td>" \
                             + "<td>Self: " + str(primer_fwd.self) + "</td></tr>"
                str_f += "</table><br>"

                for i, primer_rev in enumerate(seq.primer_set_rev.set):
                    str_r += "<td>Rev " + str(i) + ":</td><td>" + primer_rev.sequence + "</td><td> Position: " \
                             + str(primer_rev.location.start) + " - " + str(primer_rev.location.end) + "</td>" \
                             + "<td>Length: " + str(len(primer_rev)) + "</td><td>Tm: " + str(primer_rev.tm) + "</td>" \
                             + "<td>GC%: " + str(primer_rev.gc_content) + "</td>" \
                             + "<td>Any: " + str(primer_rev.any) + "</td>" \
                             + "<td>Self: " + str(primer_rev.self) + "</td>" \
                             + "<td>Penalty: " + str(primer_rev.self) + "</td></tr>"
                str_r += "</table><br>"
                pic_name = SeqPlotter.seq_plot(seq, "web_frontend/static/figures/p3seq",key)
                tab_body += "<h4>Spacing: " + seq.spacing + "; Interval: " +  seq.interval + "</h4><br>"
